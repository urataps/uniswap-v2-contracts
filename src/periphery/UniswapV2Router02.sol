...
contract UniswapV2Router02 is IUniswapV2Router02 {
    ...
    function removeLiquidityETHSupportingFeeOnTransferTokens(
        ...
        SafeTransferLib.safeTransfer(token, to, ERC20(token).balanceOf(address(this)));
        IWETH(WETH).withdraw(amountETH);
        SafeTransferLib.safeTransferETH(to, amountETH);
    }
    ...
    function swapExactTokensForTokensSupportingFeeOnTransferTokens(
        ...
        require(
            ERC20(path[path.length - 1]).balanceOf(to) - balanceBefore >= amountOutMin, "INSUFFICIENT_OUTPUT_AMOUNT"
        );
    }
